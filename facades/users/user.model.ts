...
import { sendVerificationMail, sendResetPwd } from '../emails/account';
...
async function verify(token) {
  ...
  if (user.validated === false) {
    await db.User.update({ validated: true, avatar: 'avatars/fighter.jpg' }, { where: { userID: decoded.id } }).catch(
      (e) => console.log(e)
    );
...
async function checkResetMail(mailToken) {
  ...
  const token = await db.Token.findOne({ where: { token: mailToken, userID: decoded.id } }).catch((e) =>
    console.log(e)
  );
  if (token) {
    return token;
